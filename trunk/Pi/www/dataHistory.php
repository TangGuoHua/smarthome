...
{
	case "3": // 餐厅PIR
		$query = "select CASE WHEN fldData1 = 0 THEN '无人' WHEN fldData1 = 1 THEN '有人' ELSE 'UNKNOWN' END as s1, fldCreatedOn from tabDataRecved where fldNodeID=3 order by fldID desc limit 20";
		echo "<li data-role='list-divider'>餐厅有无人最近20条记录</li>";
		break;
	case "18-1": // 次卧床头灯
		$query = "select CASE WHEN fldData1 = 0 THEN '关' WHEN fldData1 = 1 THEN '开' ELSE 'UNKNOWN' END as s1, fldCreatedOn from tabDataRecved where fldNodeID=18 order by fldID desc limit 20";
	...
	case "52": // 客厅PIR
		...
		break;
	case "55": // 大门
		$query = "select CASE WHEN fldData1 = 0 THEN '关' WHEN fldData1 = 1 THEN '开' ELSE 'UNKNOWN' END as s1, fldCreatedOn from tabDataRecved where fldNodeID=55 order by fldID desc limit 20";
		echo "<li data-role='list-divider'>大门最近20个动作</li>";
		break;
	case "111": // 书房
		$query = "select CASE WHEN fldData1 = 0 THEN '无人' WHEN fldData1 = 1 THEN '有人' ELSE 'UNKNOWN' END as s1, fldCreatedOn from tabDataRecved where fldNodeID=111 order by fldID desc limit 20";
