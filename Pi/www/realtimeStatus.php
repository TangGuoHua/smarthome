...
if ($row = $results->fetchArray()) 
{
	...
	printf( "<li><a href='dataHistory.php?param=52'>IR [%d:%d分钟] <span class='ui-li-count'>%s</span></a></li>", $row["fldData6"], $row["fldData8"], $row["fldCreatedOn"]);
...
if ($row = $results->fetchArray()) 
{
	...
	printf( "<li><a href='chart.php?param=203'>亮度 %d <span class='ui-li-count'>%s</span></a></li>", $row["fldData2"], $row["fldCreatedOn"]);	
