...
// 从DTH11读取一个字节的数据
byte getOneByte(void)
{
	byte i, temp, returnData, loopCount;
...
byte getAllBytes(void)
{
	byte temp, loopCount;
	byte tempHHigh, tempHLow, tempTHigh, tempTLow, tempCheckSum;
	byte returnData;
